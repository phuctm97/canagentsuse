...
4. Compare against the score model:
...
Return a compact table with tool, score, best interface, setup burden, sandbox status, cautions, and evidence. Then give a recommendation for the user's exact task and name the cases where a different tool would be better.
