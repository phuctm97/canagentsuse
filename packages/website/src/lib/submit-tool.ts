...
export function buildSubmitToolAgentPrompt(input: SubmitToolInput) {
  ...
  return [
    ...
    "5. The tool source file must contain a stable unique `slug`, `name`, `tagline`, `websiteUrl`, `docsUrl`, `githubUrl`, `shortDescription`, `agentSummary`, `bestFor`, `cautionNotes`, `pricingSummary`, `authModel`, `accountCreation`, `browserSupport`, `cliPackage`, `apiBaseUrl`, `mcpServer`, `launchSignals`, `categorySlugs`, `useCaseSlugs`, and `capabilities`.",
    "6. Use existing category, use-case, and capability slugs where possible. Do not invent new slugs unless the catalog schema/data already supports them or the new slug is truly required.",
    "7. For each capability, use support level `native`, `strong`, `partial`, `manual`, or `unknown`; include a concrete detail and an official/public evidence URL that matches the claim.",
    "8. Use `launchSignals` for public adoption, ecosystem importance, GitHub stars, package downloads, maturity, distribution, and evidence URL. Every `launchSignals` key must be present. Use exact public numbers when available, `null` for unavailable numeric signals, `unknown` only where allowed, and conservative tiers instead of fake or inflated values.",
    ...
    "14. Open a PR to `main` in `phuctm97/canagentsuse` using `.github/PULL_REQUEST_TEMPLATE/add-tool.md`.",
    "",
