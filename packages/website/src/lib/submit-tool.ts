...
export function buildSubmitToolAgentPrompt(input: SubmitToolInput) {
  ...
  return [
    ...
    "8. Score conservatively from evidence: machine operability, agent safety, agent readability, auth/setup, and production reliability. Add caution notes for money, production data, accounts, compliance, infrastructure, browser-only flows, or brittle automation.",
    "9. Do not add secrets, database credentials, Docker/Neon setup, generated build output, or unrelated refactors.",
    "10. Run `bun run catalog:audit`, `bun run build`, and `git diff --check`.",
