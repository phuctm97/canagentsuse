...
export function catalogToMarkdown(catalog: AgentCatalog, options?: { full?: boolean }) {
  const lines = [
    ...
    "## Agent Interfaces",
    "",
    `- [Install guide](${agentInstallLinks.guide}): Human-readable guide for adding Can Agents Use to an agent as MCP, skills.sh, or direct JSON APIs.`,
    `- [skills.sh skill](${agentInstallLinks.skillSource}): Install with \`${agentInstallLinks.skillsSh}\` or \`npx skills add phuctm97/canagentsuse --skill can-agents-use\`.`,
