...
export function catalogToMarkdown(catalog: AgentCatalog, options?: { full?: boolean }) {
  const lines = [
    ...
    "## Agent Interfaces",
    "",
    `- [Install guide](${agentInstallLinks.guide}): Human-readable guide for adding Can Agents Use to an agent as MCP, a skill, or direct JSON APIs.`,
    `- [Skill Markdown](${agentInstallLinks.skill}): Copyable SKILL.md-style instructions for agents that support persistent skills.`,
